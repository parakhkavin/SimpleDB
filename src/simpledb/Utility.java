...
public class Utility {
	 ...
	 *         strings in val, and an appended increasing integer at the end (val1,
	 *         val2, etc.).
	 */
	...
	/**
	 * @return a TupleDesc with n fields of type Type.INT_TYPE, each named name + n
	 *         (name1, name2, etc.).
	 */
	...
	/**
	 * @return a Tuple with a 'width' IntFields with the value tupledata[i] in each
	 *         field. do not set it's RecordId, hence do not distinguish which sort
	 *         of file it belongs to.
	 */
	...
	/**
	 * A utility method to create a new HeapFile with a single empty page, assuming
	 * the path does not already exist. If the path exists, the file will be
	 * overwritten. The new table will be added to the Catalog with the specified
